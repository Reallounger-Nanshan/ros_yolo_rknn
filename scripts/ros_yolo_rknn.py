...
class Yolo():
    def __init__(self):
        self.model_path = "/home/ubuntu/RosWorkSpace/rknn_ws/src/ros_yolo_rknn/weights/yolov5s.rknn"
        self.img_show = False
        ...
        self.anchor_filepath = "/home/ubuntu/RosWorkSpace/rknn_ws/src/ros_yolo_rknn/anchors/anchors_yolov5.txt"
        self.rknn = None
        self.anchors = None
        self.model = None
    ...
    def Loadimg(self, im):
        im0 = im.copy()
        im = self.LetterBox(im0, new_shape = self.img_size)
    ...
    def InitSetting(self):
        from rknnlite.api import RKNNLite
        with open(self.anchor_filepath, "r") as f:
    ...
    def Run(self, image_msg):
        ...
        ros_img = np.frombuffer(image_msg.data, dtype = np.uint8).reshape(image_msg.height, image_msg.width, -1)
        # print("Image data has been read")
        img, img0 = self.Loadimg(ros_img)
        # print("The pre-processed image is complete")
        
        outputs = self.Ratiocinate([np.expand_dims(img, axis = 0)])
        ...
        if boxes is not None:
            for box, score, cl in zip(self.GetRealBox(boxes), scores, classes):
                ...
                print("{:s} @ ({:d} {:d} {:d} {:d}) {:.3f}".format(self.cls[cl], top_left_x, top_left_y, bottom_right_x, bottom_right_y, score))
                ...
                if self.img_show:
                    ...
                    cv2.putText(drawing, "{0} {1:.2f}".format(self.cls[cl], score), (top, left - 6), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
